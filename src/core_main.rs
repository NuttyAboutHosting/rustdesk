...
pub fn core_main() -> Option<Vec<String>> {
  ...
  --device_name"#
    ...
    // NAH: on a plain UI launch, register a filename-embedded session code
    // with the support portal so the technician can connect.
    if args.is_empty() {
        crate::nah::spawn_session_reporter();
    }
    #[cfg(feature = "flutter")]
