...
def build_flutter_dmg(version, features):
    if not skip_cargo:
        # NAH: minimum osx build target is 12.3 (screencapturekit needs it; same
        # bump upstream CI applies to its arm64 job), matching the flutter xcode
        # project / Podfile / Cargo.toml pins on this branch.
        system2(
    ...
    # NAH: bundle is NAHSupport.app (PRODUCT_NAME in the Xcode project).
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/NAHSupport.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"NAHSupport Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon NAHSupport.app 200 190 --hide-extension NAHSupport.app rustdesk.dmg ./build/macos/Build/Products/Release/NAHSupport.app")
